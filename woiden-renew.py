...
TG_API_HASH = ""
TG_SESSION = ""
WOIDEN_BOT = "@HaxTG_bot"
WOIDEN_BOT_ID = 1967189265
TG_OFFICIAL_ID = 777000

WOIDEN_BASE_URL = "https://woiden.id"
WOIDEN_TITLE = "woiden.id"
WOIDEN_LOGIN_URL = f"{WOIDEN_BASE_URL}/login"
WOIDEN_VPS_INFO_URL = f"{WOIDEN_BASE_URL}/vps-info"
WOIDEN_VPS_RENEW_URL = f"{WOIDEN_BASE_URL}/vps-renew"
WOIDEN_VPS_RENEW_CODE_URL = f"{WOIDEN_BASE_URL}/vps-renew-code"
WOIDEN_VPS_STATUS_URL = f"{WOIDEN_BASE_URL}/vps-status"
WOIDEN_VPS_CONTROL_URL = f"{WOIDEN_BASE_URL}/vps-control"
    ...
    def _download_image(self, browser, src: str) -> Optional[bytes]:
        ...
        try:
            ...
            r = requests.get(src, proxies=proxies, timeout=15,
                             headers={"Referer": WOIDEN_BASE_URL + "/",
...
def woiden_click_accept(browser, idx: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        safe_screenshot(browser, shot(idx, "woiden_login_after_accept"))
        return True
    except Exception as e:
...
def woiden_login_flow(browser, phone: str, idx: int, proxy: str = None) -> bool:
    """
    ...
    for login_attempt in range(1, MAX_LOGIN_RETRIES + 1):
        ...
        print(f"\n{'─'*40}")
        print(f"  [INFO] woiden 登录流程")
        print(f"{'─'*40}")
        print(f"  [INFO] 国家代码: +{country_code}, 手机号: {mask_phone(raw)}")

        ok = _woiden_login_flow_inner(browser, country_code, phone_num, raw, idx, proxy)
        if ok:
            return True
...
def _woiden_login_flow_inner(browser, country_code: str, phone_num: str, raw: str,
                          idx: int, proxy: str = None) -> bool:
    """
    woiden 登录流程内部实现（单次尝试，不含重试）。
    ...
    Returns:
        True 登录成功，False 登录失败（由外层 woiden_login_flow 决定是否重试）
    """
    # 1. 绕过 Cloudflare
    print("  [INFO] 访问 登录页...")
    browser.open(WOIDEN_LOGIN_URL)
    time.sleep(5)
    ...
    # 滚动到 "Login to Woiden.id"
    try:
        browser.execute_script('''
            (function() {
                var els = document.querySelectorAll('h1, h2, h3, h4, div, span, p');
                for (var i = 0; i < els.length; i++) {
                    if (els[i].textContent.indexOf('Login to Woiden') !== -1) {
                        els[i].scrollIntoView({block: "start", behavior: "instant"});
                        return;
                    }
                }
        ...
        pass
    safe_screenshot(browser, shot(idx, "woiden_login_page"))
    ...
    for wait_round in range(6):
        iframe_found = browser.execute_script('''
            return !!document.querySelector('#telegram-login-LoginWoidenBot');
        ''')
        if iframe_found:
            break
        # 也尝试宽松匹配
    ...
    if not iframe_found:
        ...
        safe_screenshot(browser, shot(idx, "woiden_login_no_iframe"))
        return False

    print("  [INFO] 进入 Telegram Widget iframe...")
    try:
        tg_frame = browser.page.get_frame('css:iframe#telegram-login-LoginWoidenBot', timeout=10)
    except Exception:
        tg_frame = None
    if not tg_frame:
    ...
    if not popup_tab:
        ...
        safe_screenshot(browser, shot(idx, "woiden_login_no_tg_btn"))
        return False
    ...
    try:
        browser.execute_script('''
            (function() {
                var els = document.querySelectorAll('h1, h2, h3, h4, div, span, p');
                for (var i = 0; i < els.length; i++) {
                    if (els[i].textContent.indexOf('Login to Woiden.id') !== -1) {
                        els[i].scrollIntoView({block: "start", behavior: "instant"});
                        return;
                    }
                }
        ...
        pass
    safe_screenshot(browser, shot(idx, "woiden_login_after_iframe_click"))
    ...
    safe_screenshot(browser, shot(idx, "woiden_login_popup"))
    ...
    if not phone_code_el or not phone_el:
        ...
        safe_screenshot(browser, shot(idx, "woiden_login_no_phone_code"))
        return False
    ...
    if current_code != target_code:
        ...
        safe_screenshot(browser, shot(idx, "woiden_login_wrong_country"))
        return False

    print(f"  [INFO] 输入手机号: {mask_phone(phone_num)}")
    phone_el.input(phone_num, clear=True)
    time.sleep(1)
    safe_screenshot(browser, shot(idx, "woiden_login_phone_filled"))
    ...
    continue_btn.click()
    time.sleep(5)
    safe_screenshot(browser, shot(idx, "woiden_login_after_continue"))
    ...
    if not popup_closed:
        ...
        for wait in range(max_wait):
            ...
            if has_accept:
                ...
                woiden_click_accept(browser, idx)
                accept_clicked = True
                break
    ...
    safe_screenshot(browser, shot(idx, "woiden_login_uncertain"))
    return False


def woiden_get_vps_info(browser, idx: int) -> Tuple[List[Dict[str, str]], str, Optional[str]]:
    """访问 /vps-info/，解析服务器列表"""
    servers, seen = [], set()

    browser.open(WOIDEN_VPS_INFO_URL)
    time.sleep(5)
            ...
            if HAS_SPEECH:
                ...
                try:
                    ...
                    if attempt < max_retries:
                        ...
                        browser.open(WOIDEN_VPS_RENEW_CODE_URL)
                        time.sleep(5)
                        # Cloudflare 绕过
...
def _renew_fill_form(browser, idx: int, sid_f: str, proxy: str = None) -> None:
    """打开 /vps-renew/ 并填写表单：web_address + agreement + Turnstile + 算术验证码 + 点击提交"""
    browser.open(WOIDEN_VPS_RENEW_URL)
    time.sleep(5)
    ...
    # 否则服务器返回 "Please solve Captcha correctly!"）
    solve_arithmetic_captcha(browser, proxy=proxy, scene="续期表单提交前")
    time.sleep(1)
    ...
    if HAS_TELETHON:
        def _tg_listen():
            try:
                ...
                try:
                    extractor = TGVerificationCodeExtractor(proxy=proxy)
                    history = asyncio.run(extractor.fetch_history(WOIDEN_BOT, limit=20))
                    for h in history:
                        if not h.get("code"):
                            continue
                    ...
                    _dbg(f"TG 历史查询: {e}")
                # 再监听新消息
                code_holder[0] = tg_wait_for_code_sync(WOIDEN_BOT, timeout=150, proxy=proxy)
                if code_holder[0]:
...
def handoff_browser_to_warp(browser, target_url: Optional[str] = None) -> bool:
    ...
    cookies = [
        ...
        if 'woiden.id' in str(c.get('domain', '')).lower()
        or 'woiden.id' in str(c.get('url', '')).lower()
    ]
    try:
        old_page.quit()
    except Exception:
...
def _renew_capture_final_info(browser, result: dict, old_valid_until: str) -> None:
    ...
    try:
        servers, error, screenshot = woiden_get_vps_info(browser, result.get("_idx", 0))
        if screenshot:
...
def renew(browser, server_info: Dict[str, str], idx: int, phone: str,
    ...
    for renew_attempt in range(1, max_attempts + 1):
        ...
        try:
            # 1. 填写表单并提交
            _renew_fill_form(browser, idx, sid_f, proxy)
            ...
            if not handoff_browser_to_warp(browser, WOIDEN_VPS_RENEW_CODE_URL):
...
def logout(browser):
    try:
        browser.open(f"{WOIDEN_BASE_URL}/logout")
        time.sleep(3)
    except Exception:
        pass
...
def process(browser, phone: str, idx: int, tg_token: str = None, tg_chat: str = None,
    ...
    # 1. 登录（手机号 + TG 验证码）
    login_ok = woiden_login_flow(browser, phone, idx, proxy=proxy)
    if not login_ok:
        result["message"] = "登录失败"
...
def _send_tg_report(results, masked_phone, tg_token, tg_chat, ip_info):
    ...
    summary = f"\n" + "-" * 40 + "\n"
    summary += f"<b>📊 {WOIDEN_TITLE} 续期统计</b>\n"
...
def main():
    ...
    try:
        ...
        with open("woiden-time-value.txt", "w", encoding="utf-8") as f:
            f.write("\n".join(value_lines) + "\n")
        print("[INFO] 时间文件值已写入 woiden-time-value.txt")
    except Exception as e:
