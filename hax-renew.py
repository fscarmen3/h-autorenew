...
# 无需续期"。默认 5 天，可通过环境变量 HAX_MIN_VALID_DAYS 调整。
MIN_VALID_DAYS = int(os.environ.get("HAX_MIN_VALID_DAYS", "5").strip())
...
def hax_get_vps_info(browser, idx: int) -> Tuple[List[Dict[str, str]], str, Optional[str]]:
    ...
    m_cur = re.search(r'([A-Za-z]+ \d{1,2}, \d{4})', raw_cur)
    if m_cur:
        try:
            current_date = datetime.strptime(m_cur.group(1), "%B %d, %Y").strftime("%Y-%m-%d")
        except Exception:
            pass
    ...
    if not servers:
        ...
        if "no vps" in page_text.lower() or "no server" in page_text.lower() or "belum" in page_text.lower():
            return [], "", screenshot
        # PENDING/PROCESS：VPS 排队/开机中，无需续期
...
def _renew_fill_form(browser, idx: int, sid_f: str) -> None:
    ...
    # 解 Turnstile：若人机验证未通过，本 IP 上提交必失败，直接换 IP 重试，避免无效提交。
    if not hax_handle_turnstile(browser, idx):
        raise CaptchaBlocked("renew 表单 Turnstile/人机验证未通过")
    time.sleep(2)
